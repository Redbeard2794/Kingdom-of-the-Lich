...
ChooseRaceAndGenderMenu::ChooseRaceAndGenderMenu(sf::Font f, bool controller, int screenW, int screenH):font(f), showControllerHints(controller)
{
	//table
	...
	//parchment on table
	if (parchmentTexture.loadFromFile("Assets/CharacterCreation/parchment/parchment" + std::to_string(screenW) + "x" + std::to_string(screenH) + ".png")) {}
	else parchmentTexture.loadFromFile("Assets/CharacterCreation/parchment/parchment.png");	//if it fails load placeholder
	...
	parchment.setPosition(screenW/2, screenH/2);
	...
	genders[0].setPosition(parchment.getPosition().x - 200, parchment.getPosition().y-50);//y=500. 300,250
	...
	genders[1].setPosition(parchment.getPosition().x + 200, parchment.getPosition().y - 50);//500, 250
	...
	ChooseRace.setPosition(sf::Vector2f(screenW / 2.45, 80));
	...
	ChooseGender.setPosition(sf::Vector2f(screenW / 2.45, 80));//425
	...
	ChooseClass.setPosition(sf::Vector2f(screenW / 2.45, 80));

	//text for races
	races[HUMAN].setFont(font);
	races[HUMAN].setString("Human");
	races[HUMAN].setColor(sf::Color::Black);
	races[HUMAN].setPosition(sf::Vector2f(screenW / 5, screenH / 3));
	...
	raceIcons[0].setPosition(sf::Vector2f(screenW / 4, screenH / 4));
	...
	races[ELF].setPosition(sf::Vector2f(screenW / 2, screenH / 3));
	...
	raceIcons[1].setPosition(sf::Vector2f(screenW/2, screenH / 4));
	...
	races[DWARF].setPosition(sf::Vector2f(screenW/1.4, screenH / 3));
	...
	raceIcons[2].setPosition(sf::Vector2f(screenW/1.4, screenH / 4));
	...
	if (showControllerHints == true)//controller hints 
	{
		...
		controllerMoveHint.setPosition(screenW / 12, screenH - 30);
		...
		controllerSelectHint.setPosition(screenW - 100, screenH - 30);
		...
		controllerSelectHintText.setPosition(screenW - 260, screenH - 40);
	...
	else//keyboard and mouse hints
	{
		...
		keyboardSelectHint.setPosition(screenW - 100, screenH - 30);
		...
		keyboardSelectHintText.setPosition(screenW - 260, screenH - 40);
